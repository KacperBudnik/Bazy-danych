...
############################# Generowanie #############################
    ...
    for year in start_year+1:Dates.year(now())-1
        ...
        # Ilu dołączy
        junior_number = max(2,rand(Poisson(4)))
...
############################### Push ###############################
    ...
    begin 
        ...
        rental_table="""INSERT INTO  rental (id, rental_date, rental_date, days, customer_id, equipment_id)  VALUES\n"""
        teams_table="""INSERT INTO  teams (id, club_name, section)  VALUES\n"""
    end
    #=_table="""INSERT INTO  (id, )  VALUES(\n"""
    _table="""INSERT INTO  (id, )  VALUES(\n"""
    _table="""INSERT INTO  (id, )  VALUES(\n"""
    _table="""INSERT INTO  (id, )  VALUES(\n"""=#
    ...
    begin
        ...
        #=
        for i in 1:length(customer[1])
            txt="($(customer[1][i]),$(customer[2][i]),$(customer[3][i]))"
            txt*= i<length(customer[1]) ? ",\n" : "\n;"
            customers_table*=txt
        end
        =#
        ...
        for i in 1:length(equipment[1])
            txt="($(equipment[1][i]),$(equipment[2][i]),$(equipment[3][i]),$(equipment[4][i]),$(equipment[5][i]),$(equipment[6][i]))"
            txt*= i<length(employees[1]) ? ",\n" : "\n;"
            employees_table*=txt
        end
        =#
        ...
        for i in 1:length(rental[1])
            txt="($(rental[1][i]),'$(rental[2][i])','$(rental[3][i])',$(rental[4][i]),$(rental[5][i]),$(rental[6][i]))"
            txt*= i<length(participation[1]) ? ",\n" : "\n;"
            rental_table*=txt
        end
        =#
    ...
    reset="""SET FOREIGN_KEY_CHECKS=0;
